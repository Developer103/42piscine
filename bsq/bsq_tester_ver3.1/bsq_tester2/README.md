...
2. 文字テスト
   - empty, obstacle, fullに数字を使用（例: 1, 2, 3や0, 8, 9など）
   - empty, obstacle, fullに非表示文字を使用
   - empty, obstacle, fullに同じ文字を使用
   - 数字と記号を組み合わせて使用（例: ., 1, xなど）
...
注意: 文字の選択に制限はありませんが、以下の点に注意してください：
1. 最初の行で指定した3つの文字（empty, obstacle, full）は互いに異なる必要があります
2. 指定した3つの文字以外はマップ中で使用できません
3. 制御文字（改行、タブなど）は使用できません 